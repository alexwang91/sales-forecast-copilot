...
def build_value_report(tracking: pd.DataFrame) -> dict:
    rows = []
    for method, column in [("system", "system_p50"), ("gtm_8w_ma", "gtm_8w_ma"), ("manual", "manual_forecast")]:
        if column in tracking.columns:
            err = (tracking["actual"] - tracking[column]).abs().sum()
            den = tracking["actual"].abs().sum()
